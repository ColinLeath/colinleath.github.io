...
GFslideShow.DEFAULT_FC_OPACITY = 0.75;
...
GFslideShow.prototype.createSubContainer = function() {
  ...
  ////titlediv.style.height = '2.3em';
  //titlediv.style.height = '40px';
  //titlediv.style.fontSize = '15px';
  //titlediv.style.textAlign = 'center';
  //titlediv.style.color = '#bbbbbb';
  ////titlediv.style.float = 'right';
  ////titlediv.style.top = 1+'px';
  ////titlediv.style.left = 1+'px';
  //titlediv.style.overflow = "hidden";
  //titlediv.style.fontWeight = 'bold';
  //titlediv.style.fontFamily = 'Georgia, serif';
  ////this.container.appendChild(titlediv);

  // Hold onto our sub-container.
  this.container = div;
  //this.container.titlediv = titlediv;
};
...
GFslideShow.prototype.processThumbs = function(opt_chunk, opt_timeout) {
  ...
  for (var i = start; i < num; i++) {
    ...
    var title = this.createTitle(this.entries[i].title,this.entries[i].link);
...
GFslideShow.prototype.feedLoaded = function(result) {
  ...
  if ((this.options.pauseOnHover || this.options.fullControlPanel) &&
    ...
    this.container.onmouseover = this.bind(this.mouseOver);
    this.container.onmouseout = this.bind(this.mouseOut);
    //this.container.imageTitle.onmouseover = this.bind(this.mouseOver);
    //this.container.imageTitle.onmouseout = this.bind(this.mouseOut);
...
GFslideShow.prototype.createTitle = function(txt,link) {
  var titlelink = document.createElement("a");
  //titlelink.style.position = "absolute";
  //titlelink.style.top = 1+'px';
  //titlelink.style.left = 8+'px';
  titlelink.style.color = '#bbbbbb';
  ...
  //this.setOpacity(titlediv, 0);
  this.setOpacity(titlelink, 0);
  return titlelink;
};
...
GFslideShow.prototype.createFullControlPanel = function() {
  ...
  if (this.options.fullControlPanelCursor) {
    ...
    cursor.style.float = 'right';
    //cursor.style.textAlign = 'right';
    cursor.style.position = 'absolute';
    cursor.style.bottom = 1+'px';
    cursor.style.right = 1+'px';
...
GFslideShow.prototype.beginSlideShow = function(index) {
  ...
  this.nextTitle = this.titles[this.photo_index];
...
GFslideShow.prototype.transitionAnimation = function() {
  if (this.current && this.next) {
    ...
    //this.setOpacity(this.container.imageTitle, 1-this.next.opacity);
    this.setOpacity(this.currentTitle, cur_op);
    this.setOpacity(this.nextTitle, next_op);
...
GFslideShow.prototype.setPreviousPhoto = function() {
  ...
  while (!done && this.images.length != 0) {
    ...
    var image = this.images[this.photo_index];
    var title = this.titles[this.photo_index];
    if (image && image.__gfloaded) {
      this.next = image;
...
GFslideShow.prototype.snapToNextPhoto = function() {
  ...
  this.setOpacity(this.nextTitle, 1);
  this.setOpacity(this.currentTitle, 0);
  this.currentTitle = this.nextTitle;
  //alert(this.nextTitle);
  
  this.setDisplayTimer();
  if (this.options.transitionCallback) {
    this.options.transitionCallback(this.entries[this.photo_index],
                                    this.options.transitionTime,this.container);
  }
  //this.container.innerHTML = this.entries[this.photo_index].title
  //entry=this.entries[this.photo_index];
  //title = entry.title;
  //link = this.createLink(entry.link);
  //link.innerHTML = title;
  //this.container.imageTitle.appendChild(link);
  if (this.options.fullControlPanel && this.options.fullControlPanelCursor) {
    var index = (this.photo_index+1) + ' / ' + this.images.length;
    //this.fc.title.innerHTML = title
    //this.fc.cursor.innerHTML = title + ' (' + index + ')';
